...
if(!class_exists('membershipcron')) {
	...
	class membershipcron {
		...
		function __construct() {
			...
			$expiring_relationships = $this->get_expiring_relationships_count();
			if( ! empty( $expiring_relationships ) && $expiring_relationships >= 50 ) {
		...
		function get_expiring_relationships_count() {
			...
			if($this->db->get_var("SHOW TABLES LIKE '$this->membership_relationships'") != $this->membership_relationships) {
				return 0;
			}
