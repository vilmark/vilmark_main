...
class freesubscriptions extends Membership_Gateway {
	...
	function single_free_button( $pricing, $subscription, $user_id, $norepeat = false ) {
		...
		$form .= empty( $button )
			? '<input type="submit" class="button ' . apply_filters( 'membership_subscription_button_color', '' ) . '" value="' . __( 'Sign Up', 'membership' ) . '" />'
	...
	function display_upgrade_button($subscription, $pricing, $user_id, $fromsub_id = false) {
		...
		echo "<input type='hidden' name='user' value='" . $user_id . "' />";
		echo "<input type='hidden' name='fromsub_id' value='" . $fromsub_id . "' />";
		echo "<input type='submit' name='submit' value=' " . __('Upgrade', 'membership') . " ' class='button " . apply_filters('membership_subscription_button_color', '') . "' />";
		echo "</form>";
	}
