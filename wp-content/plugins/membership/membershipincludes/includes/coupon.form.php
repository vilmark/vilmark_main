		...
		<form method="post">
			...
			<?php if (empty($coupon) || (method_exists($coupon, 'valid_for_subscription') && !$coupon->valid_for_subscription($sub_id))) { ?>
				...
				<div class="couponEntry">
					...
					<input type='submit' class="button <?php echo apply_filters('membership_subscription_button_color', ''); ?>" id="submit_coupon_code" value = '<?php _e('Apply Coupon', 'membership'); ?>' />
				</div>
			<?php } elseif (method_exists($coupon, 'get_coupon_code') && $coupon->get_coupon_code() != false) { ?>
				...
				<?php _e('Using Coupon Code: ', 'membership'); ?>
					...
					<input type='submit' class="button <?php echo apply_filters('membership_subscription_button_color', ''); ?>" id="submit_coupon_code" value = '<?php _e('Remove Coupon', 'membership'); ?>' />
				</div>
			<?php } ?>
		</form>
	</div>
